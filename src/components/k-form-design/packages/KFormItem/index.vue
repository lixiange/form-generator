...
<template>
  ...
  >
    ...
    <KDatePicker
      ...
      v-decorator="[
        record.model, // input 的 name
        {
          initialValue: record.options.range ? record.options.rangeDefaultValue : record.options.defaultValue, // 默认值
          rules: record.rules, // 验证规则
        },
      ]"
    />
  ...
  <a-form-item
    ...
    :label-col="formConfig.layout === 'horizontal' && record.options.showLabel ? formConfig.labelCol : {}"
    :wrapper-col="formConfig.layout === 'horizontal' && record.options.showLabel ? formConfig.wrapperCol : {}"
  >
    <!-- 动态表格 -->
    <KBatch
  ...
  <a-form-item
    ...
    :wrapper-col="
      formConfig.layout === 'horizontal' ? { ...formConfig.wrapperCol, offset: formConfig.labelCol.span } : {}
    "
  >
    <a-button
      :disabled="disabled || record.options.disabled"
  ...
  >
    ...
    <img
      :style="`width:${
        record.options.alignMent === 'full' ? '105%' : '100%'
      };flex-shrink:0`"
      v-else
