  ...
  <div class="properties-centent kk-checkbox">
    ...
    <div class="properties-body">
      ...
      <a-form v-show="selectItem.key !== ''">
        ...
        <a-form-item v-if="typeof options.alignMent !== 'undefined'" label="对齐方式 (自定义需调整width)">
          <a-radio-group buttonStyle="solid" v-model="options.alignMent">
            <a-radio-button value="full">屏幕同宽</a-radio-button>
            <a-radio-button value="align">对齐表单</a-radio-button>
          </a-radio-group>
        </a-form-item>
        <a-form-item v-if="typeof options.width !== 'undefined'" label="宽度">
          <a-input placeholder="请输入" v-model="options.width" />
        </a-form-item>
        <a-form-item v-if="typeof options.topVertical !== 'undefined'" label="上间距（px）">
          <a-input-number style="width: 100%" v-model="options.topVertical" />
        </a-form-item>
        <a-form-item v-if="typeof options.bottomVertical !== 'undefined'" label="下间距（px）">
          <a-input-number style="width: 100%" v-model="options.bottomVertical" />
        </a-form-item>
        ...
        <a-form-item
          v-if="
            !['number', 'radio', 'checkbox', 'date', 'rate', 'select', 'switch', 'slider', 'html','banner'].includes(
              selectItem.type
        ...
        <!-- 上传轮播图 -->
        <a-form-item v-if="selectItem.type === 'banner'" label="上传轮播图:建议分辨率（1300*650）" extra="">
  ...
  methods: {
    imgChange(file) {
      ...
      axios({
        ...
        this.options.bannerUrl = response.data.data.url
      })
      return false
    },
    imgRemove(){
