...
<template>
  <div class="form-panel">
    <p class="hint-text" v-show="data.list.length === 0">从左侧选择控件添加</p>
    <a-form
...
export default {
  ...
  methods: {
    ...
    handleDownMerge() {
      ...
      // 计算rowspan超过自身的td
      let rows = 0
      this.rightMenuSelectValue.trs[this.trIndex].tds.forEach((element, index) => {
        if (index >= this.tdIndex) {
          return false
        }
        if (element.rowspan > this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].rowspan) {
          rows += 1
        }
      })
      if (
        this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex].colspan !==
        this.rightMenuSelectValue.trs[this.trIndex + 1].tds[this.tdIndex - rows].colspan
      ) {
      ...
      // eslint-disable-next-line standard/computed-property-even-spacing
      this.rightMenuSelectValue.trs[this.trIndex + 1].tds = this.rightMenuSelectValue.trs[this.trIndex + 1].tds.filter(
    ...
    handleRightMerge() {
      ...
      if (
        ...
        this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex + 1].rowspan
      ) {
      ...
      this.rightMenuSelectValue.trs[this.trIndex].tds[
        this.tdIndex
        // eslint-disable-next-line standard/computed-property-even-spacing
      ].colspan += this.rightMenuSelectValue.trs[this.trIndex].tds[this.tdIndex + 1].colspan

      // eslint-disable-next-line standard/computed-property-even-spacing
      this.rightMenuSelectValue.trs[this.trIndex].tds = this.rightMenuSelectValue.trs[this.trIndex].tds.filter(
        (item, index) => {
