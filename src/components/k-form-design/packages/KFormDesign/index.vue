...
import urlModal from './module/urlModal'
...
export default {
  ...
  methods: {
    ...
    handleListPush(item) {
      // 双击控件按钮push到list
      // 生成key值
      console.log(this.selectItem.key)
      console.log(item)
    ...
    handleOpenCodeModal() {
      ...
      axios({
        method: 'post',
        url: `${process.env.VUE_APP_API_BASE_URL}/api/addForm`,
        data: this.data,
      }).then((response) => {
          this.$refs.urlModal.content=`${process.env.VUE_APP_FORMPARSER_URL}/?formId=` + response.data.data
          this.$refs.urlModal.modaltitle= response.data.msg
          this.$refs.urlModal.visible=true
      })
    },
